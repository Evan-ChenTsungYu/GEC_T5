...
def pretrain_Enc(model, TT_DL,Val_DL, epochs, result_save_path, tokenizer, show_epoch_result = True, DEVICE = 'cpu', joint_training = 'none'):
    ...
    for epoch in tqdm(range(start_epoch, epochs)):
        ...
        for index, content in enumerate(TT_DL):
            ...
            text_output, class_output = model(input_data, input_mask, text_label, seperate_training = 'encoder')
            if joint_training == 'tag':
                class_label = tag_label
                class_label = class_label.float()
            elif joint_training == 'type':
                class_label = type_label
            ...
            text_loss = torch.FloatTensor([0.0]).to(DEVICE)
            total_loss = text_loss + class_loss.to(DEVICE)
            class_loss.backward()
        ...
        with torch.no_grad():
            ...
            for val_index, val_content in enumerate(Val_DL):
                ...
                if joint_training == 'tag':
                    ...
                    val_class_label = torch.ones(tag_label.size()).to(DEVICE)
                    
                # print(val_class_output.shape, val_class_label.shape)    
